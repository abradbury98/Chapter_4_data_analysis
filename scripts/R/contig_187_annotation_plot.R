...
# Update these paths to where you copied the BAKTA output files
GBFF_FILE <- "/Users/alicebradbury/Downloads/contig_187_CH06_ASC_CAT.gbff"
FASTA_FILE <- "/Users/alicebradbury/Downloads/contig_187_CH06_ASC_CAT.fna"

OUTPUT_PNG <- "/Users/alicebradbury/Desktop/contig_187_CH06_ASC_CAT_annotation.png"
OUTPUT_PDF <- NULL   # set a path here if you also want a PDF

# GC window/step (bp) — smaller values suit a ~100 kb contig
GC_WINDOW <- 2000
GC_STEP   <- 500
...
assign_category <- function(type, product) {
  ...
  case_when(
    ...
    str_detect(p, "transposase|insertion element|is\\d|mobile|integrase|recombinase site")                                  ~ "Mobile element",
    str_detect(p, "resist|efflux|beta-lactam|carbapenem|aminoglycoside|tetracycline|quinolone|macrolide|colistin|vancomycin")~ "AMR",
    ...
    str_detect(p, "dna polymerase|dna repair|recombinase|helicase|gyrase|topoisomerase|dnaa|dnab")                          ~ "DNA replication/repair",
...
genes <- genes %>% mutate(category = assign_category(type, product))
...
layout <- data.frame(
  ...
  stringsAsFactors = FALSE
) %>% mutate(offset = cumsum(c(0, head(len, -1))),
             total  = offset + len)
...
# ── 7. LABELS — AMR & MOBILE ELEMENT GENES FROM BAKTA ───────
# These come directly from the GBFF product names (no external files needed)

label_cats <- c("AMR", "Mobile element", "Virulence")

label_genes <- genes_abs %>%
  filter(category %in% label_cats, type == "CDS") %>%
  mutate(
    gene_label = coalesce(gene, locus),
...
circos.track(
  ...
  panel.fun = function(x, y) {
    ...
    # Highlight forward-strand label genes
...
text(0,  0.18, "contig_187",       cex = 1.45, font = 3, col = "white")
text(0,  0.06, "ASC_CAT_CH06",     cex = 1.15, font = 1, col = "grey80")
text(0, -0.08, paste0(n_cds, " CDSs | ", n_trna, " tRNAs"),
               cex = 0.95, col = "grey70")
